...
function RatingDistribution({ ratingsCount }: RatingDistributionProps) {
  ...
  return (
    <Stack gap="xs">
      <Text fw={600} size="sm">Rozkład ocen</Text>
      {distribution.map((item) => (
        <Group key={item.stars} justify="space-between">
          <Group gap="xs">
            <Text size="xs" w={20}>{item.stars}</Text>
            <IconStar size={14} />
          </Group>
          <Progress
            ...
            mx="md"
          />
          <Text size="xs" c="dimmed" w={30}>{item.count}</Text>
        </Group>
      ))}
    </Stack>
  );
...
export function ProjectRating({ averageRating, ratingsCount, userRating, onRate }: ProjectRatingProps) {
      ...
      <Card shadow="sm" p="md" radius="md" withBorder>
        <Stack gap="md">
          <Group justify="space-between" align="flex-start">
            <div>
              <Text fw={600} mb="xs">Ocena projektu</Text>
              <Group gap="xs">
                <Rating value={averageRating} readOnly fractions={2} />
                <Text fw={600} size="lg">{averageRating.toFixed(1)}</Text>
              </Group>
              <Group gap="xs" mt="xs">
                <IconUsers size={16} />
                <Text size="sm" c="dimmed">{ratingsCount} ocen</Text>
              </Group>
            </div>
        ...
        <Stack>
          ...
          <div>
            ...
            <Textarea
              ...
              value={review}
              onChange={(event: React.ChangeEvent<HTMLTextAreaElement>) => setReview(event.currentTarget.value)}
              minRows={3}
              maxRows={6}
            />
          </div>
