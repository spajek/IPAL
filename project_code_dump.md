...
// ZAWSZE używaj bezpośredniego API - działa tylko z klienta
const API_BASE_URL = 'https://api.sejm.gov.pl/eli'

/**
 * Pobiera listę ustaw z API Sejmu (ELI) - CLIENT SIDE ONLY
 */
export async function fakeFetchUstawy(
  ...
  const url = `/api/ustawy?${params.toString()}`
  ...
  try {
    ...
    console.error('Błąd pobierania ustaw z ELI API:', error)
    return { totalCount: 0, count: 0, offset: 0, items: [] }
  }
}

/**
 * Pobiera szczegóły konkretnej ustawy - CLIENT SIDE ONLY
 */
export async function fakeFetchActDetails(eliId: string): Promise<ActDetails | null> {
  const decodedId = decodeURIComponent(eliId)
  const parts = decodedId.split('/')

  if (parts.length < 3) {
    console.warn(`Nieprawidłowy format ELI ID: "${eliId}"`)
    return null
  }
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/acts/${publisher}/${year}/${pos}`, {
      method: 'GET',
      headers: {
        Accept: 'application/json',
      },
    ...
    const stages: LegislativeStage[] = [
      {
        stepNumber: 1,
        name: 'Data wydania aktu',
        date: item.announcementDate,
        isCompleted: !!item.announcementDate,
      },
      {
        stepNumber: 2,
        name: 'Data ogłoszenia (Promulgacja)',
        date: item.promulgation,
        isCompleted: !!item.promulgation,
      },
      {
        stepNumber: 3,
        name: 'Wejście w życie',
        date: item.entryIntoForce,
        isCompleted: !!item.entryIntoForce || item.status === 'Obowiązujący',
      },
    ]
    ...
    if (item.expirationDate) {
      stages.push({
        stepNumber: 4,
        name: 'Wygaśnięcie aktu',
        date: item.expirationDate,
        isCompleted: new Date(item.expirationDate) < new Date(),
      })
    }
    ...
    return {
      ...
      entryIntoForce: item.entryIntoForce,
      textPDF: item.textPDF,
      textHTML: item.textHTML,
    ...
    console.error('Błąd pobierania szczegółów aktu:', error)
    return null
  }
}
